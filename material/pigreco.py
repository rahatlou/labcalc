...
exp, pi = np.loadtxt('pigreco.txt', unpack=True)
...
plt.axhline( y=m.pi, color = 'red', linestyle='--', label='$\pi$ libreria matematica')
...
plt.hist(pi, color='blue', bins=12, alpha=0.5)
plt.xlabel("$\pi$ stimato")
plt.ylabel("# esperimenti")

# valore vero di pi greco
plt.axvline( x=m.pi, color = 'orange', linestyle='dotted', label='$\pi$ libreria matematica' )
# valore stimato dai dati
plt.axvline( x=mu, color = 'red', linestyle='--', label='valore medio stimato')
